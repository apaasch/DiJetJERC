...
TString study = "eta_common_fine_v5_finealpha";
...
map<TString, vector<double>> bins_alpha = {
  {"eta_common_fine_v5_finealpha", {0.05,0.075,0.10,0.125,0.15,0.175,0.20,0.225,0.25,0.275,0.30,0.325,0.35,0.375,0.40}},
};
...
map<TString, TF1*> functions = {
  ...
  {"lin03", new TF1("lin031", "[0]+[1]*x", 0, 0.31)},
...
void AlphaDependency() {
  ...
  TString out_path = path+"plots/alpha/"+study+"/";
  CreateSavePath((string) out_path);

  for(unsigned int e=1; e<bins_eta.size(); e++){
    for(unsigned int p=1; p<bins_pt.size(); p++){
      if(14<e) continue;
      eta = to_string(e);
      pt = to_string(p);
      ...
      setTDRStyle();
      int n = bins_alpha[study].size();
      double xmax = bins_alpha[study].at(n-1)+0.05;
      TCanvas *canv = tdrDiCanvas2("alpha", 0, xmax, 0.01, 0.31, 0.89, 1.11, nameXaxis, nameYaxis, "#frac{fit}{mc}", kRectangular, 0);
      canv->SetGrid();
      canv->cd(1);
      h_mc->Draw("same");
      ...
      leg->Draw();
      canv->cd(2);
      tdrDraw(new TLine(0, 1, xmax, 1));
      for(auto&r:r_mc) r.second->Draw("l same");
      canv->Print(out_path+"alpha_eta"+eta+"_pt"+pt+".pdf");
    }
  }
