...
typedef std::map<TString, VecTS> MapTS;
...
void GetValuesAndUncertainties2(VecD &SF, VecD &stat, VecD &eta_bin_center, VecD &eta_bin_error, VecDD &systematics, VecDDD & SFs, double etabins) {
  for (unsigned int i = 0; i < 1+systematics_name_all.size(); i++){
    if (i==0) {
      ...
      for (unsigned int j = 0; j < etabins; j++) {double val = (SFs.at(i).at(pt_dep_method).at(j) + SFs.at(i).at(pt_dep_method+1).at(j))/2; temp.push_back((val>2)?0:val);};
      systematics.push_back(temp);
      temp.clear();
    }
  }
...
void GetValuesAndUncertainties(VecD &SF, VecD &stat, VecD &eta_bin_center, VecD &eta_bin_error, MapD &systematics, MapDD & SFs, double etabins) {
  ...
  for (TString sys: systematics_name_all) {
    VecD temp;
    if (sys=="pTdep") {
      for (unsigned int j = 0; j < etabins; j++) {double val = (SFs[sys].at(pt_dep_method).at(j) + SFs[sys].at(pt_dep_method+1).at(j))/2; temp.push_back((val>2)?0:val);};
...
void Plot_Uncertainties2(TString name_method, VecD eta_bins, VecD eta_bin_center, VecD SF, VecD eta_bin_error, VecD stat, VecD systematics_all, VecDD systematics, VecD total_error, TString path, VecD SF_2) {
  ...
  tdrDraw(gr_tot,  "P5", kFullDotLarge, kGreen-2, kSolid, kGreen-2, 3004, kGreen-2);
  tdrDraw(gr_syst, "P5", kFullDotLarge, kBlue-4,  kSolid, kBlue-4,  3004, kBlue-4);
  tdrDraw(gr_stat, "P5", kFullDotLarge, kRed+1,   kSolid, kRed+1,   3004, kRed+1);
  leg->Draw("same");
...
void plot_SF_systematics_(TString path_ = "", TString path = "", TString year ="2018", TString QCD_DATA = "QCDPt/RunBCDEF/",TString DATA = "RunBCDEF", TString TITLE_NAME = "") {
  ...
  for (VecTS vec: {systematics_name_all,systematics_name_split}) {
    for (TString sysName: vec) {
      ...
      tdrDraw(gr_SM, "", kFullDotLarge, kRed+1, kSolid, kRed+1, 3004, kRed+1);
      tdrDraw(gr_FE, "", kFullDotLarge, kBlue-4, kSolid, kBlue-4, 3004, kBlue-4);
  ...
  tdrDraw(gr_final, "P5", kFullDotLarge, colUL16, kSolid, colUL16, 1001, colUL16, 0.15);
  // leg_final->AddEntry(map_gr["Summer16_25nsV1"], "EOY16","f");
