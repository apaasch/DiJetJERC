...
Double_t SmoothFitECALReduced(Double_t *v, Double_t *par){
  ...
  int doFit(TGraphErrors* gr1, TGraphErrors*gr2, TString lab1, TString lab2, TString leg_title, TString name, double chi2_thr, TString tag_name, bool skipZjets, bool fixP3_1, double eta_val1, double eta_val2){
    ...
    TLegend *legend = tdrLeg(0.125,0.52,0.50,0.72, 0.02, 42, kBlack);
    tdrHeader(legend,"   "+leg_title, 12);
    legend->SetTextSize(0.035);
    legend->AddEntry(gr1,lab1,"ep");
    legend->AddEntry(gr2,lab2,"ep");

    TLatex *tex2_rel = new TLatex();
    tex2_rel->SetNDC();
    tex2_rel->SetTextSize(0.035);
    tex2_rel->SetTextFont(42);

    TCanvas* c0 = tdrCanvas("SFCombined", 10,2000., 0., 3.0, "p_#T", "JER SF");
    mg->Draw("a");

    //Try with const fit first
    TF1* fit_func = new TF1("poly0","pol0",30,700);
    fit_func->SetLineColor(kBlack);
    TFitResultPtr fit_SF = mg->Fit(fit_func,"FQNR");
    float chi2 = fit_func->GetChisquare()/fit_func->GetNDF();
    TString chi2_val_str;
    chi2_val_str.Form("%3.1f",chi2);
    TString chi2_rel = "constant fit #chi^{2}/ndf = ";
    ...
    if(chi2>chi2_thr){
      ...
      TF1* fit_func_custom = new TF1("SF_custom",SmoothFitECALReduced,30,700,4);
      fit_func_custom->SetLineColor(kBlack);
      ...
      for (int i=0; i<gr1->GetN(); i++) grint1->SetPoint(i, gr1->GetX()[i], 0);
      /*Compute the confidence intervals at the x points of the created graph*/
      (TVirtualFitter::GetFitter())->GetConfidenceIntervals(grint1);
      for (int i=0; i<gr1->GetN(); i++) grint1->SetPoint(i, gr1->GetX()[i], fit_func_custom->Eval(gr1->GetX()[i]));
      for (int i=0; i<gr2->GetN(); i++) grint2->SetPoint(i, gr2->GetX()[i], 0);
      /*Compute the confidence intervals at the x points of the created graph*/
      (TVirtualFitter::GetFitter())->GetConfidenceIntervals(grint2);
      for (int i=0; i<gr2->GetN(); i++) grint2->SetPoint(i, gr2->GetX()[i], fit_func_custom->Eval(gr2->GetX()[i]));

      chi2 = fit_func_custom->GetChisquare()/fit_func_custom->GetNDF();
      chi2_val_str.Form("%3.1f",chi2);
      TString chi2_rel = "step fit #chi^{2}/ndf = ";
      chi2_rel +=chi2_val_str;
      ...
      tex2_rel->DrawLatex(0.65,0.30,chi2_rel);
      OutputSFs(fit_func_custom, name, eta_val1, eta_val2);
    } else {
      tex2_rel->DrawLatex(0.65,0.30,chi2_rel);

      TFitResultPtr fit_SF = mg->Fit(fit_func,"FR");

      /*Create a TGraphErrors to hold the confidence intervals*/
      for (int i=0; i<gr1->GetN(); i++) grint1->SetPoint(i, gr1->GetX()[i], 0);
      /*Compute the confidence intervals at the x points of the created graph*/
      (TVirtualFitter::GetFitter())->GetConfidenceIntervals(grint1);
      for (int i=0; i<gr1->GetN(); i++) grint1->SetPoint(i, gr1->GetX()[i], fit_func->Eval(gr1->GetX()[i]));
      for (int i=0; i<gr2->GetN(); i++) grint2->SetPoint(i, gr2->GetX()[i], 0);
      /*Compute the confidence intervals at the x points of the created graph*/
      (TVirtualFitter::GetFitter())->GetConfidenceIntervals(grint2);
      for (int i=0; i<gr2->GetN(); i++) grint2->SetPoint(i, gr2->GetX()[i], fit_func->Eval(gr2->GetX()[i]));
    ...
    tdrDraw(gr1, "pe", kFullTriangleDown, colr, kSolid, colr, 1001, colr);
    tdrDraw(gr2, "ep", kFullTriangleUp, colb, kSolid, colb, 1001, colb);
    tdrDraw(grint1, "e3", 20, colr, kSolid, colr, 1001, colr);
    tdrDraw(grint2, "e3", 22, colb, kSolid, colb, 1001, colb);
    gr1->SetLineColor(colr);
    gr2->SetLineColor(colb);
    gr1->SetMarkerColor(colr);
    gr2->SetMarkerColor(colb);
    gr1->SetMarkerSize(1.2);
    gr2->SetMarkerSize(1.2);
    grint1->SetFillColorAlpha(colr,0.3);
    grint2->SetFillColorAlpha(colb,0.3);
  ...
  void CombineDijetZjets(TString tag_name="Ngenjet"){
    ...
    TString path_dijet = "/nfs/dust/cms/user/amalara/WorkingArea/UHH2_102X_v1/CMSSW_10_2_10/src/UHH2/DiJetJERC/JERSF_Analysis/JER/wide_eta_binning/file/MergeL2Res/Autumn18_V19/AK4CHS/standard/QCDHT/RunABCD/output/"; //V17
    ...
    TString path_zjet = "ZJets_JER_combination/August19/abs_eta/";
    ...
    for(int ieta=0;ieta< n_eta_bins-1;ieta++){
      ...
      TString legend_title = "#eta^{jet} #in ["+eta_bins_str[ieta]+", "+eta_bins_str[ieta+1]+"]";
      ...
      int fitres = doFit(gr_zjets, gr_dijets, "Z+jet", "Dijet",legend_title,"JERSF_Eta_"+eta_bins_name[ieta],1.5, tag_name, skipZjets, fix_Par3_1, eta_bins[ieta], eta_bins[ieta+1]);
    }
