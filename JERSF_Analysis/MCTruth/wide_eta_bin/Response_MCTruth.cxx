...
int Response_MCTruth(){
  ...
  files["QCDHT50to100"]    = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT50to100_UL18.root");
  files["QCDHT100to200"]   = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT100to200_UL18.root");
  files["QCDHT200to300"]   = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT200to300_UL18.root");
  files["QCDHT300to500"]   = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT300to500_UL18.root");
  files["QCDHT500to700"]   = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT500to700_UL18.root");
  files["QCDHT700to1000"]  = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT700to1000_UL18.root");
  files["QCDHT1000to1500"] = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT1000to1500_UL18.root");
  files["QCDHT1500to2000"] = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/"+study+"/"+corr+"/"+label+"/uhh2.AnalysisModuleRunner.MC.QCDHT1500to2000_UL18.root");
  // files["QCDHT2000toInf"]  = new TFile("/nfs/dust/cms/user/paaschal/sframe_all/DiJetJERC_DiJetHLT/UL18/eta_common/Summer19UL18_V5/AK4CHS/uhh2.AnalysisModuleRunner.MC.QCDHT2000toInf_UL18.root");
  ...
  for(unsigned int e=0; e<vec_eta_bins.size(); e++){
    ...
    for(unsigned int p=0; p<ptbins.size(); p++){
      ...
      // cout << ebin << " " << pbin << endl;
      hists["response"][ebin][pbin] = new TH1F("response_eta"+ebin+"_pt"+pbin, "response_eta"+ebin+"_pt"+pbin, 200, 0, 2.0); hists["response"][ebin][pbin]->Sumw2();
      hists["ptgen"][ebin][pbin]    = new TH1F("ptgen_eta"+ebin+"_pt"+pbin, "ptgen_eta"+ebin+"_pt"+pbin, 150, 0, 1500);      hists["ptgen"][ebin][pbin]->Sumw2();
    }
  }

  for(auto f: files){
    cout << "Start processing " << f.first << " ... " << endl;
    auto start = chrono::high_resolution_clock::now(); // Calculation time - start
    fillHists(files[f.first]);
  ...
  outpath += "/src/UHH2/DiJetJERC/JERSF_Analysis/MCTruth/wide_eta_bin/files/";
  outpath += study+"/";
  outpath += corr+"/";
  outpath += label+"/";
  TFile *fout = new TFile(outpath+"/MCTruth_JER.root", "recreate");
  fout->cd();
  for(auto h: hists){
